...
def downgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("digest_id", sa.Integer(), nullable=False),
        sa.Column("created_at", sa.DateTime(), nullable=False),
        sa.UniqueConstraint("user_id", "news_item_id", name="uq_news_item_digest_coverage_user_item"),
    )
    op.create_index(
    ...
    op.add_column(
        "users",
        sa.Column("news_digest_timezone", sa.String(length=100), nullable=False, server_default="UTC"),
    )
    op.add_column(
        "users",
