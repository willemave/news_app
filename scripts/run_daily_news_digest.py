...
def main() -> None:
    ...
    with bound_log_context(job_name="run_daily_news_digest", trigger="cron", source="cron"):
        logger.info(
            "Daily digest cron run started",
            extra=build_log_extra(
                component="cron",
                operation="run_daily_news_digest",
                event_name="cron.run",
                status="started",
                job_name="run_daily_news_digest",
                trigger="cron",
                context_data={
                    "dry_run": args.dry_run,
                    "lookback_hours": args.lookback_hours,
                    "now_utc": now_utc.isoformat(),
                    "requested_user_ids": args.user_ids,
                },
            ),
        )
        ...
        with get_db() as db:
            ...
            for user in query.yield_per(200):
                ...
                if args.dry_run:
                    logger.info(
                        "[dry-run] Would enqueue daily digest",
                        extra=build_log_extra(
                            component="cron",
                            operation="run_daily_news_digest",
                            event_name="cron.run",
                            ...
                            context_data={
                                ...
                                "interval_hours": interval_hours,
                                "coverage_end_at": target.coverage_end_at.isoformat(),
                            },
                        ),
                    )
                    continue
        ...
        logger.info(
            "Daily digest cron run completed",
            extra=build_log_extra(
                component="cron",
                operation="run_daily_news_digest",
                event_name="cron.run",
                ...
                context_data={
                    ...
                    "considered_count": users_considered,
                    "enqueued_count": task_ready_count,
                    "skipped_count": existing_digest_count,
                    "failed_count": 0,
                    "due_now_count": users_due_now,
                },
            ),
        )
