...
def dump_content_table(console: Console) -> None:
    ...
    with get_db() as db:
        ...
        content_table = Table(title=f"Content Records (Latest {min(20, total_count)})", show_header=True)
...
def dump_processing_tasks_table(console: Console) -> None:
    ...
    with get_db() as db:
        ...
        tasks_table = Table(title=f"Processing Tasks (Latest {min(20, total_count)})", show_header=True)
...
def main():
    ...
    if len(sys.argv) > 1:
        # Detailed view mode
        if len(sys.argv) != 3:
            console.print("[red]Usage for detailed view: python dump_database.py <table> <id>[/red]")
