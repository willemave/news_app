    ...
    with SessionLocal() as db:
        try:
            ...
            if days:
                ...
                print(f"Filtering to content errored since {cutoff_date.strftime('%Y-%m-%d %H:%M:%S')} UTC")
            ...
            if dry_run:
                ...
                for content in errored_content[:20]:  # Show first 20 in dry run
                    print(f"  - ID: {content.id}, Type: {content.content_type}, Source: {content.source}")
            ...
            deleted_tasks = db.query(ProcessingTask).filter(
            ...
            for content in errored_content:
                ...
                task = ProcessingTask(
                    ...
                    payload={
                        ...
                        "original_error": content.error_message[:500] if content.error_message else None
                    }
                )
