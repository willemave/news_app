...
| `client/newsly/newsly/Models/DailyNewsDigest.swift` | `struct DailyNewsDigest`, `struct DailyNewsDigestBulletDetail`, `struct DailyNewsDigestCitation`, `struct DailyNewsDigestListResponse` | News-native digest run models used by the short-form tab and `/api/news/digests` responses. |
...
| `client/newsly/newsly/Models/OpenAI.swift` | `struct AudioTranscriptionResponse`, `enum CodingKeys`, `encode` | Uploaded-audio transcription response model. |
