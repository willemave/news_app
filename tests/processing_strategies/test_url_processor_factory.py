...
def mock_http_client(mocker):
    ...
    mock.head = AsyncMock(return_value=AsyncMock(spec=httpx.Response, headers=httpx.Headers()))
    return mock
...
def mock_html_strategy_class(mocker):
    ...
    instance_mock = AsyncMock(spec=HtmlProcessorStrategy) # Mock for instances
    instance_mock.can_handle_url = AsyncMock(return_value=False)
...
def mock_pdf_strategy_class(mocker):
    ...
    instance_mock = AsyncMock(spec=PdfProcessorStrategy)
    instance_mock.can_handle_url = AsyncMock(return_value=False)
...
def mock_pubmed_strategy_class(mocker):
    ...
    instance_mock = AsyncMock(spec=PubMedProcessorStrategy)
    instance_mock.can_handle_url = AsyncMock(return_value=False)
    mock.return_value = instance_mock
...
async def test_get_strategy_selects_pubmed(
    ...
    pubmed_instance_mock.can_handle_url = AsyncMock(return_value=True) # This instance will say yes

    url = "https://pubmed.ncbi.nlm.nih.gov/1234567/"
    strategy = await factory_with_mocked_strategies.get_strategy(url) # Use the new fixture

    assert isinstance(strategy, AsyncMock) # Should be our mocked instance
...
async def test_get_strategy_selects_pdf_by_content_type(
    factory_with_mocked_strategies: UrlProcessorFactory,  # Use the new fixture
    mock_http_client: MagicMock,
    mock_pdf_strategy_class,
    ...
    pubmed_instance_mock.can_handle_url = AsyncMock(return_value=False)
    html_instance_mock.can_handle_url = AsyncMock(return_value=False)

    async def pdf_can_handle_logic(url_param, response_headers=None):
        ...
        if response_headers and response_headers.get('Content-Type') == 'application/pdf':
            return True
        return False
    pdf_instance_mock.can_handle_url = AsyncMock(side_effect=pdf_can_handle_logic)

    url = "http://example.com/document" # URL doesn't end in .pdf
    mock_head_response = AsyncMock(spec=httpx.Response)
    mock_head_response.headers = httpx.Headers({'Content-Type': 'application/pdf'})
    mock_head_response.url = url
    mock_http_client.head = AsyncMock(return_value=mock_head_response)

    strategy = await factory_with_mocked_strategies.get_strategy(url) # Use the new fixture
    
    assert isinstance(strategy, AsyncMock)
...
async def test_get_strategy_selects_html_by_content_type(
    factory_with_mocked_strategies: UrlProcessorFactory, # Use the new fixture
    mock_http_client: MagicMock,
    mock_html_strategy_class,
    mock_pubmed_strategy_class, # Added fixture
    ...
    # Ensure other strategies say NO
    pubmed_instance_mock.can_handle_url = AsyncMock(return_value=False)
    pdf_instance_mock.can_handle_url = AsyncMock(return_value=False)
    
    async def html_can_handle_logic(url_param, response_headers=None):
        ...
        if response_headers and response_headers.get('Content-Type') == 'text/html':
            return True
        return False
    html_instance_mock.can_handle_url = AsyncMock(side_effect=html_can_handle_logic)
...
async def test_get_strategy_selects_html_as_fallback(
    factory_with_mocked_strategies: UrlProcessorFactory, # Use the new fixture
    mock_http_client: MagicMock,
    mock_html_strategy_class,
    ...
    async def always_false_can_handle(url_param, response_headers=None):
        return False
    pubmed_instance_mock.can_handle_url = AsyncMock(side_effect=always_false_can_handle)
    pdf_instance_mock.can_handle_url = AsyncMock(side_effect=always_false_can_handle)
    
    # HTML strategy says yes, but only when called with headers (after HEAD)
    async def html_fallback_logic(url_param, response_headers=None):
    ...
    html_instance_mock.can_handle_url = AsyncMock(side_effect=html_fallback_logic)
...
async def test_get_strategy_no_strategy_found(
    factory_with_mocked_strategies: UrlProcessorFactory, # Use the new fixture
    mock_http_client: MagicMock,
    mock_html_strategy_class,
    mock_pdf_strategy_class,
    ...
    mock_http_client.head = AsyncMock(side_effect=httpx.UnsupportedProtocol("FTP not supported"))
...
async def test_get_strategy_head_request_fails(
    factory_with_mocked_strategies: UrlProcessorFactory, # Use the new fixture
    mock_http_client: MagicMock,
    ...
    async def always_false_for_url_check(url_param, response_headers=None):
        if response_headers is None: # URL-only check
            return False
        return False # Default for other cases
    
    pubmed_instance_mock.can_handle_url = AsyncMock(side_effect=always_false_for_url_check)
    pdf_instance_mock.can_handle_url = AsyncMock(side_effect=always_false_for_url_check)
    ...
    async def html_can_handle_after_head_fail(url_param, response_headers=None):
        ...
        if url_param == "http://example.com/fallback_url" and response_headers is None:
            ...
            if html_instance_mock.can_handle_url.call_count > 1: # call_count is 1-based
                return True
        return False
    html_instance_mock.can_handle_url = AsyncMock(side_effect=html_can_handle_after_head_fail)

    url = "http://example.com/fallback_url"
    mock_http_client.head = AsyncMock(side_effect=httpx.RequestError("Network error during HEAD"))
...
async def test_factory_registration(mock_http_client: MagicMock):
    ...
    class DummyStrategy(UrlProcessorStrategy):
        async def can_handle_url(self, url: str, response_headers: httpx.Headers | None = None) -> bool: return "dummy_url" in url
        async def download_content(self, url: str): return "dummy_content"
        async def extract_data(self, content, url: str): return {"text_content": content}
        async def prepare_for_llm(self, extracted_data): return extracted_data

    factory_instance.register_strategy(DummyStrategy)
    
    # Check if DummyStrategy is called for its specific URL
    mock_dummy_instance = AsyncMock(spec=DummyStrategy)
    mock_dummy_instance.can_handle_url = AsyncMock(return_value=True)
    ...
    with patch.object(DummyStrategy, '__init__', return_value=None) as mock_dummy_init:
        ...
        strategy_returned = await factory_instance_for_reg_test.get_strategy("http://example.com/dummy_url")
        assert isinstance(strategy_returned, DummyStrategy)

        strategy_not_found = await factory_instance_for_reg_test.get_strategy("http://example.com/other_url")
