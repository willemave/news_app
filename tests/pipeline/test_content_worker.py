...
def mock_dependencies():
    ...
    with patch("app.pipeline.worker.get_checkout_manager") as mock_checkout, \
...
class TestContentWorker:
    ...
    def test_process_content_sync_not_found(self, mock_dependencies):
        ...
        result = worker.process_content_sync(123, "test-worker")
    ...
    def test_process_article_sync_success(self, mock_dependencies):
        ...
        # Mock HTTP service
        worker.http_service.fetch_content_sync.return_value = ("<html>content</html>", {})
        
        # Mock LLM service
        mock_summary = Mock()
        mock_summary.model_dump.return_value = {"summary": "Test summary"}
        worker.llm_service.summarize_content_sync.return_value = mock_summary
        ...
        worker.http_service.fetch_content_sync.assert_called_once_with("https://example.com/article")
        mock_strategy.extract_data.assert_called_once()
        worker.llm_service.summarize_content_sync.assert_called_once_with("This is test content.")
    ...
    def test_process_article_sync_non_retryable_error(self, mock_dependencies):
        ...
        worker.http_service.fetch_content_sync.side_effect = NonRetryableError(
    ...
    def test_process_podcast_sync_success(self, mock_dependencies):
        ...
        worker.podcast_download_worker.process_download_task_sync.return_value = True
        worker.podcast_transcribe_worker.process_transcribe_task_sync.return_value = True
    ...
    def test_process_unknown_content_type(self, mock_dependencies):
        ...
        with patch("app.pipeline.worker.content_to_domain") as mock_converter:
            ...
            result = worker.process_content_sync(789, "test-worker")
