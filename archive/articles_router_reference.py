...
def get_daily_articles(
    ...
    available_dates_query = (
        db.query(func.date(Articles.scraped_date).label("date"))
        .distinct()
        .order_by(func.date(Articles.scraped_date).desc())
    )
...
def detailed_article(request: Request, article_id: int, db: Session = Depends(get_db)):
    article = (
        db.query(Articles)
        .join(Links, Articles.link_id == Links.id)
        .filter(Articles.id == article_id)
        .first()
    )
    return templates.TemplateResponse(
        "detailed_article.html", {"request": request, "article": article}
    )
