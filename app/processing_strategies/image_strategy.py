...
class ImageProcessorStrategy(UrlProcessorStrategy):
        ...
        if response_headers:
            ...
            for mime_type in self.IMAGE_MIME_TYPES:
                if mime_type in content_type:
                    logger.debug(f"ImageStrategy can handle {url} based on Content-Type: {content_type}")
                    return True
    ...
    def prepare_for_llm(self, extracted_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        logger.info(f"ImageStrategy: Preparing empty data for LLM (image will be skipped)")
        return {
            "content_to_filter": "",
