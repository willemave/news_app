...
class UrlProcessorFactory:
    ...
    async def get_strategy(self, url: str) -> Optional[UrlProcessorStrategy]:
        ...
        try:
            ...
            logger.info(f"Factory: HEAD request for {url} successful. Content-Type: {response_headers.get('content-type')}. Final URL: {head_response.url}")
        except Exception as e:
            logger.warning(f"Factory: HEAD request for {url} failed: {e}. Will try strategies without headers.")
