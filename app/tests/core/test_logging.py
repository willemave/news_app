...
class TestGenericErrorLoggerIntegration:
    ...
    def test_log_error_calls_logger_exception(self):
        ...
        with patch("app.utils.error_logger.get_logger") as mock_get_logger:
            ...
            assert call_kwargs["extra"]["component"] == "test_component"
            assert call_kwargs["extra"]["operation"] == "test_operation"
            assert call_kwargs["extra"]["context_data"] == {"key": "value"}
            assert call_kwargs["extra"]["item_id"] == 123
            assert call_kwargs["extra"]["error_type"] == "ValueError"
            assert call_kwargs["extra"]["error_message"] == "Test error message"
    ...
    def test_log_http_error_includes_url_in_context(self):
        ...
        with patch("app.utils.error_logger.get_logger") as mock_get_logger:
            ...
            assert "url" in call_kwargs["extra"]["context_data"]
            assert call_kwargs["extra"]["context_data"]["url"] == "https://example.com/api"
    ...
    def test_log_processing_error_includes_item_id(self):
        ...
        with patch("app.utils.error_logger.get_logger") as mock_get_logger:
            ...
            assert call_kwargs["extra"]["item_id"] == 456
            assert call_kwargs["extra"]["operation"] == "transform"
