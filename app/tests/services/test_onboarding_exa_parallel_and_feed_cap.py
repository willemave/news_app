...
def test_fast_discover_defaults_caps_feed_suggestions_to_ten() -> None:
    response = _fast_discover_from_defaults(_many_feed_defaults())
    assert len(response.recommended_substacks) == 10


def test_build_discovery_response_caps_feed_suggestions_to_ten() -> None:
